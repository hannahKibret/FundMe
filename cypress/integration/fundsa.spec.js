...
describe("View all fundraisers as an admin", () => {
    ...
    it('getting fundraisers from the server', () => {   
         cy.log('intercepting /fundraisers/')
         cy.intercept('GET','/api/fundraisers/',req => {
      ...
      it('getting fundraisers from fixture and checking its length', () => {
        cy.intercept('GET','/api/fundraisers/',mergeResponse({ fixture: 'fundraisers.json' })).as('funds')
        cy.getByTestId("view-fund").click()
      ...
      it('block a fundraiser', () => {
        cy.intercept('GET','/api/fundraisers/').as('funds')
        cy.getByTestId("view-fund").click()
        cy.wait('@funds')
        cy.intercept('PUT','**/api/fundraisers/*').as('blk-fund')
        cy.get('div.container').last().contains('BLOCK').last().click()
        cy.wait('@blk-fund').its('response.body').should('include', {
            isBlocked: true
        }) 

      })

      it('closing fundraiser list page of the admin', () => {
        cy.intercept('GET','/api/fundraisers/').as('funds')
        cy.getByTestId("view-fund").click()
        cy.wait('@funds')
        cy.get('.xbtn').click()
