...
const FormFundraise = ({ submitForm ,location}) => {
  const { handleChange, handleChangee,handleFileChange,handleSubmit, values, errors,categories,imagePath,category } = useFundraiseForm(
    submitForm,
    validate,
    location
  );
  ...
  return (
    <div className='form-content-right'>
      <form onSubmit={handleSubmit} className='form' encType = "multipart/form-data" noValidate>
        ...
        <img className="img-fund" src={(imagePath === null)?(values.image):(imagePath)}/>
        <div className='form-inputs'>
        ...
        <div className='form-inputs'>
          ...
          <select value={category} name="category"  className='form-dropdown' onChange={handleChangee}>
            {
               categories
               .map(cat =>
